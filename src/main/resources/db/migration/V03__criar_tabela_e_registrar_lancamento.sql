CREATE TABLE `lancamento` (
	...
	`observacao` VARCHAR(100) COLLATE 'utf8_general_ci',
	`tipo` VARCHAR(20) NOT NULL COLLATE 'utf8_general_ci',
	`codigo_categoria` BIGINT(20) NOT NULL,
	`codigo_pessoa` BIGINT(20) NOT NULL,
	PRIMARY KEY (`codigo`) USING BTREE,
	INDEX `codigo_categoria` (`codigo_categoria`) USING BTREE,
	INDEX `codigo_pessoa` (`codigo_pessoa`) USING BTREE,
	CONSTRAINT `FK_lancamento_categoria` FOREIGN KEY (`codigo_categoria`) REFERENCES `algamoney`.`categoria` (`codigo`) ON UPDATE RESTRICT ON DELETE RESTRICT,
	CONSTRAINT `FK_lancamento_pessoa` FOREIGN KEY (`codigo_pessoa`) REFERENCES `algamoney`.`pessoa` (`codigo`) ON UPDATE RESTRICT ON DELETE RESTRICT
)
